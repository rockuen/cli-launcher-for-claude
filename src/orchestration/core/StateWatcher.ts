...
import { parseHudJson } from './hudHelpers';

export { parseHudJson } from './hudHelpers';
...
export class StateWatcher extends EventEmitter {
  ...
  start(projectRoot: string): void {
    ...
    this.readAndEmit();
    this.logger(`[cli-launcher.hud] watching ${path.join(projectRoot, RELATIVE_HUD_CACHE)}`);
  }
  ...
  private readAndEmit(): void {
    ...
    const parsed = parseHudJson(raw);
    if (parsed === null) return; // partial write or empty
