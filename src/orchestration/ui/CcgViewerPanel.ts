...
export class CcgViewerPanel {
  static readonly viewType = 'claudeCodeLauncher.ccgViewer';
  ...
  private constructor(
    ...
    panel.webview.onDidReceiveMessage(async (msg: unknown) => {
      ...
      if (m.type === 'copy' && typeof m.text === 'string') {
        ...
        vscode.window.setStatusBarMessage('CLI Launcher: CCG output copied', 2000);
        return;
      }
      if (m.type === 'open-source' && typeof m.text === 'string') {
        try {
          ...
          vscode.window.showWarningMessage(
            `CLI Launcher: could not open ${m.text} — ${err instanceof Error ? err.message : err}`,
          );
        }
        return;
      }
      if (m.type === 'rerun' && typeof m.pairId === 'string') {
        ...
        try {
          ...
          this.output.appendLine(`[claudeCodeLauncher.ccg] rerun failed: ${err}`);
        }
      }
    });
  }
...
<title>CLI Launcher · CCG Viewer</title>
<style>
