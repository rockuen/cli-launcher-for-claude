...
export const COLOR_IDS = {
  claude: 'claudeCodeLauncher.agent.claude',
  codex: 'claudeCodeLauncher.agent.codex',
  gemini: 'claudeCodeLauncher.agent.gemini',
  omc: 'claudeCodeLauncher.brand.omc',
  statusRunning: 'claudeCodeLauncher.status.running',
  statusDone: 'claudeCodeLauncher.status.done',
  statusFailed: 'claudeCodeLauncher.status.failed',
  statusCancelled: 'claudeCodeLauncher.status.cancelled',
  statusIdle: 'claudeCodeLauncher.status.idle',
} as const;
...
const PROVIDER_BASE_HSL: Record<ConversationProvider, { h: number; s: number; l: number }> = {
  ...
  // #FB923C (orange) — matches --ccl-omc / --accent-leader
