...
test('No CollapsibleState.None on session/trash item rows', () => {
  ...
  // + gjc + chief session items, the unified view's other-agent session leaf
  // (_loadOtherAgentItems builds kiro/codex/grok/gjc/chief/agy leaves for the merged tree),
  ...
  assert.ok(
    noneCount <= 11,
    `expected <= 11 None usages (metadata row + kiro session + kiro trash leaf + antigravity session leaf + codex session leaf + grok session leaf + gjc session leaf + chief session leaf + unified other-agent leaf + search no-match row), got ${noneCount}`,
  );
});
