...
const { CLAUDE_DIR, PROFILES_DIR, CLAUDE_JSON_PATH, CREDENTIALS_PATH, keychain } = vi.hoisted(() => {
  ...
  return {
    ...
    PROFILES_DIR: _path.join(claudeDir, "account-switcher"),
...
describe("listProfiles", () => {
  it("returns [] when account-switcher dir doesn't exist", () => {
    expect(listProfiles()).toEqual([]);
  });

  it("returns [] when account-switcher exists but is empty", () => {
