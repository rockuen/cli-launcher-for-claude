...
- **상태 + 알림** — 응답이 진행 중이면 탭과 패널 테두리가 **노란색으로 글로우**, 인터랙티브 프롬프트로 입력이 필요하면 탭이 **파란 waiting 글리프**, 끝나면 **초록색**으로 바뀐다. 시스템 알림도 같이 띄워서 다른 일 하다가도 자연스럽게 알아챈다.
...
**두 번째, 응답 상태가 한눈에 보여야 한다**. AI가 깊게 생각할 때는 1분 넘게 걸린다. 그동안 끝났는지 매번 창을 보러 가는 게 맥을 끊었다. 그래서 **상태를 시각적으로 박았다** — 응답이 진행 중이면 탭과 패널 테두리가 **노란색으로 글로우**, 입력이 필요하면 탭이 **파란 waiting 글리프**, 끝나면 **초록색**으로 바뀐다. 시스템 알림도 같이 띄운다. 다른 일을 하다가도 옆눈으로 색만 흘끗 보면 무엇이 attention을 요구하는지 알 수 있다.
...
- **상태 인식 탭** — idle / running / done / error / needs-attention. 인터랙티브 프롬프트(`[Y/n]`, 메뉴 등)는 7초 임계 무시하고 즉시 needs-attention + 파란 waiting 탭 아이콘 + 데스크탑 알림.
