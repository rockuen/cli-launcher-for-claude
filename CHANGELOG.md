...
### Fixed
- **Scrolling back through a terminal pane was undone by the next line of output.** With the reader off, the panel pinned the terminal to the bottom after *every* PTY chunk, so wheeling up to re-read an earlier answer lasted only until the agent printed its next line — which, for a running agent, is constantly. The pin came from a flat `PIN_TERMINAL_TO_BOTTOM = true`, which made the "was the user at the bottom?" probe the code already ran dead code: `shouldPinTerminal()` short-circuited before ever looking at it.
- The terminal now follows the live bottom only while the viewport is already parked there, which is what a terminal normally does: scroll up to pause the follow, scroll back down to resume it. The position is re-probed before every write, so resuming needs nothing but scrolling back to the bottom. This restores what v3.10.4 did for kiro before v3.12.0's blanket pin covered it over.
- **Every refit now restores the bottom itself, which is what the pin was really doing.** v3.12.0 added the pin for "stale upper rows after output/resize", and that symptom belongs to the refit paths, not to output: a `fitAddon.fit()` reflows scrollback and can leave the viewport off the bottom, and the pin was silently correcting that on the next chunk. Five refit paths had no guard of their own — font size, font family, the settings split-ratio slider, the split-ratio clamp, and `fitTerm()`, which backs splitter drags **and the terminal auto-expand that fires when an interactive prompt appears**. Without the pin behind them, a reflow there would have stranded the user above a `/model`-style menu with no way back except the scroll button. All of them now probe before the fit and restore the bottom when that is where the user was; four share one `refitTerminal()` helper, and `fitTerm()` probes outside its rAF because by the time the frame runs the layout has already moved.
- **The scroll-to-bottom button now appears exactly when the follow is paused.** It judged position with a 10px threshold while the follow policy compares buffer rows, so the two could disagree by a row — the follow stopped while the only affordance that resumes it stayed hidden. Both now use the same probe, and output re-checks it, since new rows can change the answer without firing a DOM scroll event.

### Notes
- No carve-out was needed for full-screen TUIs or for grok, and the earlier draft of this entry claiming one was wrong on two counts. Full-screen TUIs are not all scrollback-less — kiro renders a full-screen frame in the *normal* buffer with the standard 5000-line scrollback (v3.10.4) — and where scrollback genuinely is absent (the alternate screen, grok's `scrollback: 0`) the pin happens for free: xterm sizes such a buffer to exactly `rows`, so `ybase` never leaves 0 and the probe reads "at the bottom" on every call. An explicit branch for those cases would have been unreachable code.
- Reading the buffer is not the approach that failed in v3.7.3/v3.7.13. That one tracked scroll intent through the `onScroll` **event**, which a TUI's own redraw fires exactly as a user's wheel does — indistinguishable, so the follow flag flipped off and kiro stuck at the top. The buffer state is unambiguous: `ydisp < ybase` is reachable only through `BufferService.scrollLines(negative)`, whose callers are all real user input (viewport wheel, PageUp/PageDown, selection drag, accessibility, public API). Escape sequences go through `scroll()`, which moves `ydisp` up together with `ybase` and never sets `isUserScrolling`.

### Tests
- 13 cases in `terminalScroll.test.ts`. The one that matters most walks **every** `fitAddon.fit()` in the client and fails unless it probes the viewport first and restores the bottom after — the initial fit, which runs before any output exists, is the single documented exception. Removing any one guard makes it fail (verified). The rest: a scrolled-up viewport not being dragged down, a bottom-parked one following, the resume path, the unknown-position fallback, the policy running for real per agent rather than being string-matched, inline-safety of the embedded function (no backticks, interpolation, or `require`), guards that the hard-pin constant is gone and that nothing short-circuits past the probe, the reader toggle going through the shared helper, the scroll button and the write callback sharing the probe, and a `node --check` parse of the rendered client per agent. Full suite passes (577 Node tests, 64 vitest).
...
### Added
- **Usage status bar (model + limits).** A new right-aligned status bar item shows, inline, the focused session's **model**, the **5-hour** window (`5h NN% ↺<time>`), and the **weekly** window (`wk NN% ↺<weekday time>`) — each with its reset time in local time. On plans that expose per-model weekly limits the weekly segment splits into `wk·Opus` / `wk·Sonnet`; otherwise it shows the single account weekly. The tooltip adds the full per-window breakdown (5h / 7d / Opus / Sonnet) with reset date+time and a relative countdown. This is the launcher-side equivalent of the model + usage line Gajae Code prints in its TUI. The item hides when no Claude OAuth credentials exist; clicking it (or running **"Refresh Claude Usage / Limits"**) forces an immediate refetch.
...
### Implementation
- `pty/resolveCli.js`: `resolveGjcCli()` + `resolveChiefCli()`. `agents/registry.js`: `gjc` entry (`label: 'Gajae'`, `cliName: 'gjc'`) + `chief` entry (`label: 'Chief'`, `cliName: 'chief'`). `lib/projectSessions.js`: `gjcAgentDir` / `gjcSessionsDir` / `getGjcPaths` + `GJC_CODING_AGENT_DIR` env prep, plus `chiefHome` / `chiefSessionsDir` / `getChiefPaths` + Chief env injection. `lib/sessionJsonl.js`: `listGjcSessions` / `findGjcSessionPath` / `_extractGjcMessages` and `listChiefSessions` / `findChiefSessionPath` / `_extractChiefMessages` + `getSessionJsonlPath` / `extractAiTitle` / `extractMessages` / `extractMessageCount` wiring. `panel/createPanel.js` + `panel/restartPty.js`: gjc spawn / resume-by-path / continue + `--model` / `--thinking` on fresh sessions + the fresh-session discovery/pin watcher (`_claimedGjcIds`), and the chief spawn / resume branches. `tree/SessionTreeDataProvider.js`: `gjc` and `chief` store-key namespaces, MIME pairs, `_buildGjcSessions` / `_buildChiefSessions`, unified `UNIFIED_OTHER_AGENTS` entries, drag/sort wiring. `activation.js`: `newGjc` + `gjc.pickModel` + `gjc.setupCredentials` + `newChief` commands, `gjcTreeProvider` / `chiefTreeProvider` + views, `gjcAvailable` / `chiefAvailable` context keys, resume branches. `handlers/gjcModel.js` (new): the model/subscription picker + credential setup. `bin/chief-repl.js` (new): the Chief REST PTY wrapper. `package.json`: agent + enabledAgents enums, `gjc.model` / `gjc.thinking` + `chief.*` settings, Gajae + Chief Sessions views + menus + view-title buttons, reader names.
